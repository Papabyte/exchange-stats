<template>
	<div>
		...
		<div class="box">
			...
			<div class="column" v-if="searchedAddress">
				<div v-if="wallets[found_address_wallet]" >
				...
				<div v-else-if="found_address_wallet" class="columns">
					<div class="column">
					<span class="title is-5">
							<i18n class="vert-bottom" path="explorerWalletsForExchangeWalletDoesntBelongToExchange">
								<template #address>
									{{searchedAddress}}
								</template>
								<template #wallet>
									<wallet-id :id="Number(found_address_wallet)" />
								</template>
								<template #exchange>
									{{exchangeName}}
								</template>
					...
					<div class="column" >
						<b-button
							...
							type="is-warning"
							icon-left="plus"
							@click="addWalletToExchange(found_address_wallet,request_input)"
						>
						Add to {{exchangeName}}
						</b-button>
					</div>
				...
				<div v-else>
					<span class="title is-5">{{$t('explorerWalletsForExchangeUnknownWalletDoesntBelongToExchange', {address: searchedAddress, exchange: exchangeName})}} </span>
				</div>
			</div>
		</div>
...
export default {
	...
	methods: {
		...
		addWalletToExchange(walletId, exchange){
			ModalProgrammatic.open({
				parent: this,
		...
		searchAddress(){
			...
			if (!this.address_input) {
				...
				this.text_error = $t("explorerWalletsForExchangeMetaDescriptionNotValidAddress")
			...
			this.axios.get('/api/address/' + this.address_input).
				then((response) => {
						...
						console.log(this.found_address_wallet )
				})
			}
		}
	}
