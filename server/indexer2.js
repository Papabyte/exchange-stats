...
console.log(inputs);
			...
			tx.vout.forEach(function(output){
					if (output.scriptPubKey.addresses.length == 1){
						outputs.push({address: output.scriptPubKey.addresses[0], value: Math.ceil(output.value*10000000), n: output.n });
					}else {
						throw Error("more than one address " + tx.txid);
			...
			if (inputs.length == 0){ // coinbase
				(async function(){
					await saveOnlyTransactionsTo(firstInputsTreated, tx.txid, objBlock.height, outputs);
			...
			(async function(){
				inputs = await getInputAddressesIdAndValueIn(inputs);
				await saveInputAddressesAndTransactions(firstInputsTreated, inputs, tx.txid, objBlock.height , outputs, tx_index);
				firstInputsTreated = false;
				await mergeWallets(inputs, objBlock.height,objBlock.time, tx_index);// we merge these input addresses in the wallet id that has already the most addresses and set up redirections
				callback();
			})();
		}, function(){
...
function getInputAddressesIdAndValueIn(inputs){
	return new Promise(async function(resolve){
		const sqlFilter =	inputs.map(function(input){ return " (transactions.tx_id='"+input.tx_id+"' AND transactions_to.n="+input.vout+")";}).join(' OR ');
		const rows = await db.query("SELECT transactions_to.address_id, SUM(amount) as value_in FROM transactions INNER JOIN transactions_to USING(id) WHERE "+ sqlFilter);
		var input_address_ids = rows.map(function(row){return row.address_id});
		resolve({input_address_ids:input_address_ids, value_in: rows[0].value_in});
		});
}
...
function saveInputAddressesAndTransactions(bFirstTreated, inputs, tx_id, height, outputs, tx_index){
	return new Promise(async function(resolve){
		if (bFirstTreated){
			console.error("first treated");
			const results = await db.query("SELECT 1 FROM transactions WHERE tx_id=?",[tx_id]);
		...
		db.takeConnectionFromPool(function(conn) {
			...
			for (var i=0; i<outputs.length; i++){
				conn.addQuery(arrQueries,"INSERT OR IGNORE INTO btc_addresses (address) VALUES (?)",[outputs[i].address]);
				conn.addQuery(arrQueries,"REPLACE INTO transactions_to(id, wallet_id, address_id, amount,n) \n\
			...
			var InputaddressesSqlString = inputs.input_address_ids.join(",");
			if (InputaddressesSqlString.length> 1)
				console.error("-------------------- InputaddressesSqlString.length " + InputaddressesSqlString);
			conn.addQuery(arrQueries,"INSERT INTO btc_wallets (addr_count) VALUES (?-(SELECT COUNT(ROWID) FROM btc_addresses WHERE wallet_id IS NOT NULL AND id IN("+InputaddressesSqlString+")))",[inputs.input_address_ids.length]);	//increase address count only for addresses that are not already known
			conn.addQuery(arrQueries,"UPDATE btc_addresses SET wallet_id=(SELECT MAX(id) FROM btc_wallets) WHERE wallet_id IS NULL AND id IN("+InputaddressesSqlString+")");
				...
				((SELECT MAX(id) FROM transactions),(SELECT MAX(id) FROM btc_wallets),?)",[inputs.value_in]);
...
function mergeWallets(inputs,block_height, block_time, tx_index){
	return new Promise(async function(resolve){
		if (inputs.input_address_ids.length > 1){
			db.takeConnectionFromPool(async function(conn) {
				...
				IN(SELECT DISTINCT(wallet_id) FROM btc_addresses \n\
					WHERE address_id IN("+InputaddressesSqlString+"))\n\
