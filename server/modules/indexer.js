...
function saveInputAddressesAndTransactions(bFirstTreated, objInputs, tx_id, height, outputs, tx_index){
	return new Promise(async function(resolve){
		...
		db.takeConnectionFromPool(function(conn) {
			...
			conn.addQuery(arrQueries,"UPDATE btc_wallets SET txs_count=txs_count+1 \n\
			WHERE id IN (SELECT DISTINCT(wallet_id) FROM btc_addresses WHERE address IN('"+outputAddressesSqlString+"') AND id NOT IN("+inputAddressesSqlString+"))");// tx from input will be counted at merge step
