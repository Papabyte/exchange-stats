...
function saveInputAddressesAndTransactions(bFirstTreated, input_addresses, tx_id, height, value_in, outputs, tx_index){
	return new Promise(async function(resolve){
		...
		db.takeConnectionFromPool(function(conn) {
			...
			conn.addQuery(arrQueries,"INSERT INTO btc_wallets (addr_count) VALUES (?-(SELECT COUNT(ROWID) FROM btc_addresses WHERE address IN('"+InputaddressesSqlString+"')))",input_addresses.length );
	...
	return new Promise(async function(resolve){
		if (input_addresses.length > 1){
			db.takeConnectionFromPool(async function(conn) {
				...
				conn.addQuery(arrQueries, "UPDATE transactions_to INDEXED BY toByAddress SET wallet_id=?,address=NULL WHERE address IN('"+InputaddressesSqlString+"')", [rows[0].id]);
